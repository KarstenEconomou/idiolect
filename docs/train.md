...
The tracked TOML example contains the complete experiment policy. Python code does not select a model, seed, data format, optimizer value, adapter target, path, or external report service.
...
The example uses `mlx-community/Qwen3-14B-4bit` at a fixed repository revision. The first run downloads the model to `train.model_cache`. The model is approximately 8.3 GB.

Do not put a model hub token in TOML. Put `HF_TOKEN` in `.env` only if the model requires authentication.

## Configure

Copy the current `[train]` tables from `conf/local.toml.example` to the private `conf/local.toml` file. Review every value before a run.
...
- `seeds`: Select each independent run. The example runs three adapters in sequence.
...
The example uses Qwen non-thinking text. It adds `/no_think` to each prompt and an empty thinking block before each completion. This change occurs only in the private run copy. The immutable canonical dataset does not change.
